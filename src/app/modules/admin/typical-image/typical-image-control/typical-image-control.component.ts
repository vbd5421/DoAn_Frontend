...
export class TypicalImageControlComponent {
  ...
  pick(e: any) {
    this.target.url = this.baseURL + '/' + this.tImageURL + '/image/' + e.image.name;
    this.target.name = e.image.originalFileName;
    this.target.id = e.id;
  ...
  updateTimage(id: number) {
    return this.router.navigate([`admin/typical-image/edit/${id}`])
  }
